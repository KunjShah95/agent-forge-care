          ...
          <Card className="bento-card p-6">
            {!hasResumes ? (
              ...
              <>
                ...
                {(atsData.github_demonstrated_skills?.length > 0 || atsData.github_inferred_skills?.length > 0) && (
                  <div className="mb-4 p-4 rounded-xl border border-emerald-500/20 bg-emerald-500/5">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h4 className="text-xs font-semibold text-emerald-600 dark:text-emerald-400">GitHub Verified Skills</h4>
                    </div>
                    <div className="flex flex-wrap gap-1.5 mb-2">
                      {atsData.github_demonstrated_skills?.map((s: string) => (
                        <Badge key={s} className="bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20 text-xs">
                          {s} <span className="ml-1 opacity-60">(proven)</span>
                        </Badge>
                      ))}
                      {atsData.github_inferred_skills?.map((s: string) => (
                        <Badge key={s} variant="outline" className="text-xs border-emerald-500/20 text-muted-foreground">
                          {s} <span className="ml-1 opacity-60">(inferred)</span>
                        </Badge>
                      ))}
                    </div>
                    {atsData.project_evidence && atsData.project_evidence.length > 0 && (
                      <details className="text-xs text-muted-foreground group">
                        <summary className="cursor-pointer hover:text-foreground transition-colors">
                          {atsData.project_evidence.length} project{atsData.project_evidence.length > 1 ? 's' : ''} with evidence
                        </summary>
                        <ul className="mt-2 space-y-1 pl-2">
                          {atsData.project_evidence.map((p: string, i: number) => (
                ...
                {/* ── Portfolio Skills ── */}
                {(atsData.portfolio_skills?.length > 0 || atsData.portfolio_projects?.length > 0) && (
                  <div className="mb-4 p-4 rounded-xl border border-violet-500/20 bg-violet-500/5">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h4 className="text-xs font-semibold text-violet-600 dark:text-violet-400">Portfolio Data</h4>
                      {atsData.portfolio_role && (
                        <Badge variant="outline" className="text-xs border-violet-500/20">
                          {atsData.portfolio_role}
                        </Badge>
                      )}
                    </div>
                    {atsData.portfolio_skills && atsData.portfolio_skills.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {atsData.portfolio_skills.map((s: string) => (
                          <Badge key={s} variant="outline" className="text-xs border-violet-500/20 text-muted-foreground">
                            {s}
                          </Badge>
                        ))}
                      </div>
                    )}
                    {atsData.portfolio_projects && atsData.portfolio_projects.length > 0 && (
                      <details className="text-xs text-muted-foreground group">
                        <summary className="cursor-pointer hover:text-foreground transition-colors">
                          {atsData.portfolio_projects.length} portfolio project{atsData.portfolio_projects.length > 1 ? 's' : ''}
                        </summary>
                        <ul className="mt-2 space-y-1 pl-2">
                          {atsData.portfolio_projects.map((p: string, i: number) => (
                            <li key={i} className="flex items-start gap-1.5">
